...
- `## Quick Wins` — `- [ ] item | route: now / delegate(<who>) / later [| src: <url>]`
...
Rollover rules: Wins & Growth shows the MOST RECENT business day with completed wins. At /checkin, after logging the current day's wins, move the previously-shown (prior business day's) wins to WINS-ARCHIVE-<year>.md so the section then shows today's. NEVER clear or roll wins during /focus — /focus does not roll wins; only /checkin rolls them (when new wins are logged). Big Rocks Wins holds only rocks completed in the last 30 days; at the 30-day rollover, move older entries to WINS-ARCHIVE-<year>.md. Ritual State is updated in place each ritual, never appended to. LOG-ARCHIVE-<year>.md is read-only history.
...
2. Classify each thread/line into ledger entries WITH the user. Ask on ambiguous type. Type scale: `quick_win` = a single same-day task, roughly 20 minutes to an hour; `big_rock` = multi-hour or multi-day, usually multi-step. Dated big_rocks get a due date + next physical action. `due: none` triggers the commitment question: "Real commitment (keep, undated) or park it?" `recurring` items file to `## Recurring` with a `next due` date — ask the user for the recurrence cadence if unclear. Wins are captured only through the /checkin conversation ("what did you finish?") — never scan email for wins. Waiting For follow-ups are logged manually by the user or stated in conversation (e.g., /checkin capture) — never mined from sent mail.
