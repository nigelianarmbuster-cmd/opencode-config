...
- `## Log` — hidden (never shown in the dashboard); NOT ritual bookkeeping (that's Ritual State) and NOT wins (that's Wins & Growth/Big Rocks Wins). Used ONLY for notable decision/provenance records worth keeping (e.g., a user-set presumption, a structural change, a rule change) — one sparse line per event, dated `- YYYY-MM-DD — note`. Keep it short; roll old entries to LOG-ARCHIVE.md when it grows.
